...
rec {
  ...
  */
  isRgb =
    ...
    let
      ...
      ++ lib.optionals (color ? a) [ color.a ];
    in
    lib.lists.all (v: isValidRGB v) colors;
        ...
        builtins.map
          (
            ...
            let
              c' = self.math.round c;
            in
            self.hex.pad 2 (self.hex.fromDec c')
          )
          [
            r
            g
  ...
  */
  fromHex =
    ...
    let
      ...
      a = if lib.lists.length hex' == 4 then lib.lists.elemAt hex' 3 else null;
    in
    RGB {
      inherit r g b;
  ...
  */
  lighten =
    ...
    percentage:
    let
      grow' = c: self.math.grow' valueMin valueMax percentage;
    in
    RGB {
      r = grow' r;
      g = grow' g;
  ...
  */
  hexMatch =
    ...
    let
      ...
      generateRegex = r: n: lib.strings.replicate r "([[:xdigit:]]{${builtins.toString n}})";
    ...
    in
    if (length == 3 || length == 4) then builtins.map (x: scale x) output else output;
}
