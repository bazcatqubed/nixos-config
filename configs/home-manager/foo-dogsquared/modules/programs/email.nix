...
in
{
  options.users.foo-dogsquared.programs.email = {
    ...
    himalaya.enable = lib.mkEnableOption "foo-dogsquared's email client on the command line";
    aerc.enable = lib.mkEnableOption "foo-dogsquared's TUI email client";
  };
    ...
    lib.mkMerge [
      {
        accounts.email.accounts = {
          ...
          old_personal = rec {
            ...
            userName = address;
            flavor = "gmail.com";
            passwordCommand = "gopass show websites/accounts.google.com/christiangabrielarazas | head -n 1";
          };
        };
      }
