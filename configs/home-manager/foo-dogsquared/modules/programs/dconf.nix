...
in
{
  options.users.foo-dogsquared.programs.dconf.enable = lib.mkEnableOption "dconf configuration";
    ...
    lib.mkMerge [
      {
        dconf.settings = {
          ...
          "org/gnome/epiphany".homepage-url =
            lib.mkIf userCfg.programs.custom-homepage.enable "file://${config.xdg.dataHome}/foodogsquared/homepage/index.html";
        };
      }
    ]
  );
