  ...
  nvimPkg = createNixvimFlavor (
    { config, ... }:
    {
      ...
      config = {
        ...
        plugins.treesitter.grammarPackages = lib.mkForce config.plugins.treesitter.package.passthru.allGrammars;
      };
    }
  );
...
in
{
  options.users.foo-dogsquared.programs.nixvim.enable = lib.mkEnableOption "editors made with NixVim";
