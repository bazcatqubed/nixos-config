...
in
{
  options.users.foo-dogsquared.setups.desktop = {
    enable = lib.mkEnableOption "a set of usual desktop productivity services";
    comics.enable = lib.mkEnableOption "setup for storing and reading comic books";
  };
  ...
  config = lib.mkIf cfg.enable (
    lib.mkMerge [
      ...
      (lib.mkIf cfg.comics.enable {
        home.packages = with pkgs; [
          komikku
        ];

        state.ports.comic-server.value = 7954;

        home.services.comic-server = {
          imports = [
            foodogsquaredLib.services.suwayomi-server.default
          ];

          suwayomi-server = {
            dataDir = config.home.homeDirectory;
            settings.server = {
              port = config.state.ports.comic-server.value;
              systemTrayEnabled = false;
              initialOpenInBrowserEnabled = false;

              downloadAsCbz = true;
              downloadsPath = "${config.xdg.userDirs.documents}/Comics";
        ...
        xdg.dataFile."Tachidesk/server.conf".source =
          config.home.services.comic-server.configData."server.conf".source;

        users.foo-dogsquared.programs.custom-homepage.sections =
          lib.mkIf (config.home.services.comic-server or false != { })
            {
              services.links = lib.singleton {
                url = "http://localhost:${builtins.toString config.state.ports.comic-server.value}";
                text = "Comic book server";
              };
