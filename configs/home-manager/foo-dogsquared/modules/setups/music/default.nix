    ...
    lib.mkMerge [
      {
        home.packages = with pkgs; [
          ...
          vcv-rack # A gay synthesizer, I think?
        ];
        ...
        users.foo-dogsquared.programs.custom-homepage.sections = lib.mkMerge [
          (lib.mkIf (attrs.nixosConfig.services.gonic.enable or false) (
            let
              subsonicLink = {
      ...
      (lib.mkIf cfg.spotify.enable {
        xdg.autostart.entries = lib.singleton (foodogsquaredLib.xdg.getXdgDesktop pkgs.spotify "spotify");
      ...
      (lib.mkIf cfg.mpd.enable {
        ...
        sops.secrets."mopidy/listenbrainz" =
          foodogsquaredLib.sops-nix.getAsOneSecret ./mopidy-listenbrainz.secrets.bin;
          ...
          settings =
            ...
            in
            {
              ...
              file = {
                ...
                ++ lib.optional (isFilesystemSet "external-hdd") "${attrs.nixosConfig.state.paths.external-hdd}/Music|External storage"
                ++ lib.optional (isFilesystemSet "archive") "${attrs.nixosConfig.state.paths.archive}/Music|Archive";
              };
