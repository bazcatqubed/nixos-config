      ...
      jobsList = lib.map (
        ...
        in
        {
          ...
          value = {
            ...
            urls = lib.map (subscription: subscription.url) category.data.subscriptions;
...
in
{
  options.users.foo-dogsquared.setups.research = {
    enable = lib.mkEnableOption "foo-dogsquared's usual toolbelt for research";

    writing.enable = lib.mkEnableOption "writing suite";
  };
    ...
    lib.mkMerge [
      {
        ...
        xdg.autostart.entries = lib.singleton (foodogsquaredLib.xdg.getXdgDesktop pkgs.zotero "zotero");
