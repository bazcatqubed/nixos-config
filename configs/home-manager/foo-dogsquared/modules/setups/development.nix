...
in
{
  options.users.foo-dogsquared.setups.development = {
    ...
    creative-coding.enable = lib.mkEnableOption "foo-dogsquared's creative coding setup";

    gamedev.enable = lib.mkEnableOption "foo-dogsquared's gamedev suite";

    math.enable = lib.mkEnableOption "foo-dogsquared's meth suite";
  };
    ...
    lib.mkMerge [
      {
        ...
        users.foo-dogsquared.programs.custom-homepage.sections.services.links =
          ...
          in
          lib.optionals hasCockpitEnabled (
            lib.singleton {
