...
in
{
  ...
  config = lib.mkIf (hostCfg.networking.enable && cfg.enable) (
    lib.mkMerge [
      {
      ...
      (lib.mkIf (networkSetup == "networkd") {
        ...
        sops.secrets =
          let
            ...
            applySystemdAttr =
              secretPaths:
              lib.listToAttrs (lib.map (path: lib.nameValuePair path systemdNetworkFileAttrs)) secretPaths;
          in
          applySystemdAttr [
            "wireguard/private-key"
            "wireguard/preshared-keys/phone"
