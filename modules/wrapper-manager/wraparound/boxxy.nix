...
let
  ...
  boxxyModuleFactory =
    ...
    {
      package =
        lib.mkPackageOption pkgs "boxxy" { } // lib.optionalAttrs (!isGlobal) { default = cfg.package; };
  ...
  options.wrappers =
    let
      boxxySandboxModule =
        ...
        in
        {
          options.wraparound.variant = lib.mkOption { type = with lib.types; nullOr (enum [ "boxxy" ]); };
            ...
            wraparound.boxxy.extraArgs =
              ...
              ++ (lib.mapAttrsToList (
                ...
                let
                  ...
                  ruleArg = if mode != null then "${source}:${destination}:${mode}" else "${source}:${destination}";
                in
                "--rule ${ruleArg}"
              ) submoduleCfg.rules);
