  ...
  bubblewrapModuleFactory =
    ...
    }:
    {
      dbus = {
        ...
        filter = {
          package =
            lib.mkPackageOption pkgs "xdg-dbus-proxy" { }
...
in
{
  options.wraparound.bubblewrap = lib.recursiveUpdate (bubblewrapModuleFactory { isGlobal = true; }) {
            ...
            wraparound.bubblewrap.dbus.filter.extraArgs =
              let
                makeDbusProxyArgs =
                  ...
                  [
                    address
                    (builtins.toString metadata.path)
                  ]
                  ++ metadata.extraArgs;
              in
              lib.lists.flatten (lib.mapAttrsToList makeDbusProxyArgs submoduleCfg.dbus.filter.addresses);

            wraparound.bubblewrap.sharedNixPaths = [ submoduleCfg.dbus.filter.package ];
          };
        };
    in
    lib.mkOption {
