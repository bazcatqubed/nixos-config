            ...
            lib.mkMerge [
...

              {
                wraparound.bubblewrap.filesystem =
                  let
                    renameNixStorePaths = path: if lib.isDerivation path then path.pname else path;
                    makeFilesystemMapping =
                      operation: bind:
                ...
                wraparound.bubblewrap.extraArgs =
                  let
                    makeFilesystemArgs =
                      ...
                      ++ (
                        if
                          lib.elem metadata.operation [
                            "dir"
                            "remount-ro"
                          ]
                        then
                          [ "--${metadata.operation} ${dst}" ]
                        else
                          [ "--${metadata.operation} ${src} ${dst}" ]
                      )
              ...
              (lib.mkIf submoduleCfg.enableSharedNixStore {
                ...
                ++ lib.optionals (builtins.storeDir != "/nix/store") [ "/nix/store" ];
              })
              ...
              (lib.mkIf (submoduleCfg.sharedNixPaths != [ ]) {
                wraparound.bubblewrap.extraArgs =
                  ...
                  in
                  lib.map (p: "--ro-bind ${p} ${p}") closurePaths;
              })
