...
let
  ...
  settingsFormat = {
    ...
    generate = name: value: pkgs.writeTextDir "/dconf/${name}" (lib.generators.toDconfINI value);
  };
  ...
  dconfModuleFactory =
    ...
    {
      enable =
        lib.mkEnableOption "configuration with dconf"
        // lib.optionalAttrs (!isGlobal) { default = cfg.enable; };

      package =
        lib.mkPackageOption pkgs "dconf" { } // lib.optionalAttrs (!isGlobal) { default = cfg.package; };
    ...
    let
      dconfSubmodule =
        ...
        let
          ...
          dconfProfileFile = pkgs.writeText "dconf-profile" (
            lib.concatMapStrings (db: ''
              ${db}
            '') submoduleCfg.profile
          );

          dconfSettings = settingsFormat.generate "wrapper-manager-dconf-${config.executableName}-settings" submoduleCfg.settings;
