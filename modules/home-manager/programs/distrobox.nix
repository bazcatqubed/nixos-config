...
in
{
  options.programs.distrobox = {
    settings = lib.mkOption {
      ...
      default = { };
      description = ''
        User-wide settings for Distrobox to be generated at
        {file}`$XDG_CONFIG_HOME/distrobox/distrobox.conf`.
  ...
  config = lib.mkIf cfg.enable {
    xdg.configFile."distrobox/distrobox.conf".source = settingsFile;
  };
}
