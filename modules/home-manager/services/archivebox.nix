...
in
{
  options.services.archivebox = {
    ...
    withDependencies = lib.mkEnableOption "additional dependencies to be installed";
  ...
  config =
    let
      ...
      ++ (lib.optionals cfg.withDependencies (
        with pkgs;
        [
          chromium
          nodejs_latest
    ...
    lib.mkIf cfg.enable {
      assertions = [
        (lib.hm.assertions.assertPlatform "services.archivebox" pkgs lib.platforms.linux)
      ];
      ...
      systemd.user.services = lib.mkMerge [
        ...
        (lib.mkIf cfg.webserver.enable {
          archivebox-server = {
            ...
            Service = {
              ExecStart = "${pkgs.archivebox}/bin/archivebox server localhost:${toString cfg.webserver.port}";
              WorkingDirectory = cfg.archivePath;
              Restart = "on-failure";
            };
          };
