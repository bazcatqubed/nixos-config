              ...
              modules = [
                (lib.mkIf hasHMBranch (
                  { lib, ... }:
                  {
                    ...
                    home-manager.extraSpecialArgs = partsConfig.setups.home-manager.sharedSpecialArgs;
                  }
                ))
                  ...
                  { lib, pkgs, ... }:
                  {
                    config = lib.mkMerge [
                        ...
                        nixpkgs.overlays =
                          let
                            hmUsersOverlays = lib.mapAttrsToList (
                              name: _: partsConfig.setups.home-manager.configs.${name}.nixpkgs.overlays
                            ) setupConfig.home-manager.users;
                      ...
                      (lib.mkIf (isNixpkgs "separate") {
                        ...
                        home-manager.users = lib.mapAttrs (name: _: {
                          nixpkgs.overlays = partsConfig.setups.home-manager.configs.${name}.nixpkgs.overlays;
                        }) setupConfig.home-manager.users;
                      })
                    ];
                  }
    ...
    (lib.mkIf (cfg.configs != { }) {
      flake =
        ...
        in
        {
          ...
          deploy.nodes =
            let
              validConfigs = lib.filterAttrs (name: _: cfg.configs.${name}.deploy != null) pureHomeManagerConfigs;
