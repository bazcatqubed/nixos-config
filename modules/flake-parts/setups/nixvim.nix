...
let
  ...
  configType =
    ...
    }:
    {
      ...
      config = {
        modules = [ "${partsConfig.setups.configDir}/nixvim/${config.configName}" ];
  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf (cfg.configs != { }) {
      perSystem =
        ...
        (
          let
            validConfigs = lib.filterAttrs (_: metadata: lib.elem system metadata.systems) cfg.configs;

            nixvimConfigurations =
              let
                generateNixvimConfigs =
                  name: metadata:
                  let
                    mkNixvimConfig' =
                      component:
                      ...
                      in
                      lib.nameValuePair "${name}-${component.nixpkgsBranch}-${neovimPackage.pname}" (mkNixvimConfig {
                        inherit system pkgs;
