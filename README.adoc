...
* Deploy the same NixOS configuration for more than one system, in multiple output images generated and as part of the deployment cluster with https://github.com/serokell/deploy-rs[deploy-rs].
...
This project follows https://reuse.software/spec-3.3/[REUSE specification] with most of its files already have the licensing information embedded infile.
For those who care, the overwhelmingly majority of the project is licensed under MIT license.
